...
//    public static final String GOOGLE_API_KEY = "AIzaSyDLToSD04V2ylySZaswsUWZX9s-WCGDx5g";
    public static final String GOOGLE_API_KEY = "AIzaSyCZtq9W1uQQOQAjNV7Vv01v2k0ffqFBow4";
    ...
    public static void onCreateGPS(final Application context){
        ...
        Logger.I("AlxLocationManager","准备开启gps");
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                Logger.I("AlxLocationManager","该手机没有安装谷歌框架服务,使用Android原生获取吧");
                ...
                try {
                    ...
                    Log.i("AlxLocationManager","开启locationtask出现异常",e);
                }
    ...
    public static class LocationTask extends TimerTask {
        ...
        @Override
        public void run() {
            Logger.I("AlxLocationManager","location task 执行"+"manager是"+manager+"    destroy是"+AlxLocationService.isDestory);
    ...
    public static void pauseGPS(){
        Logger.I("AlxLocationManager","准备暂停GPS");
    ...
    public void onConnected(@Nullable Bundle bundle) {
        Logger.I("AlexLocation","connect gps成功");
        ...
        if(!getCurrentLocation()) {//得到当前gps并记录
            ...
            Logger.I("AlxLocationManager","成功获取到了上次的经纬度，并进行跟踪");
            ...
            Logger.I("AlxLocationManager","成功获取到了上次的经纬度，所以不进行追踪了");
    ...
    class TowerAndWiFiTask extends AlxAsynTask<Void,Void,String>{
        ...
        protected String doInBackground(Void... params) {
            ...
            String json = geoLocationAPI.toJson();//这里使用gson.toJson()会被混淆，推荐使用手动拼json
            Logger.I("AlxLocationManager","准备发给goggle的json是"+json);
            return json;
        }
    ...
    public boolean getCurrentLocation(){
        ...
        Logger.I("AlxLocationManager","得到last Location的gps是=="+mLastLocation);
        ...
        Logger.I("AlxLocationManager","last Location的精度是"+last_accuracy);
        ...
        Logger.I("AlxLocationManager","获取last location成功，纬度=="+latitude+"  经度"+longitude+"  海拔"+altitude+"   传感器"+provider+"   速度"+speed+ "精确度"+last_accuracy);
        if(last_accuracy < MAX_deviation){
            recordLocation(context,latitude,longitude,last_accuracy);
        }else {
            Logger.I("AlxLocationManager","精确度太低，放弃last Location");
        }
    ...
    public void onLocationChanged(Location location) {
        ...
        Logger.I("AlxLocationManager","位置改变了"+location);
        ...
        if(location.getAccuracy() < MAX_deviation ){//精度如果太小就放弃
            ...
            Logger.I("AlxLocationManager","精确度太低，准备放弃最新的位置");
        }
        ...
        Logger.I("AlxLocationManager","准备开启省电策略");
    ...
    public static void recordLocation(Context context, double latitude, double longitude, float accuracy){
        ...
        editor.apply();
        Logger.I("AlxLocationManager","最终经过火星转换的结果是latitude="+latitude+"   longitude="+longitude+"   accuracy="+accuracy);
        ...
        if(myLocationStatic.updateTime == 0 || System.currentTimeMillis() - myLocationStatic.updateTime > SLOW_INTERVAL || accuracy <= myLocationStatic.accuracy) {
            ...
            Logger.I("AlxLocationManager","本次位置获取不精确，放弃");
        }
    }
    ...
    public static double[] getOldLocation(Context context){
        ...
        Logger.I("AlxLocationManager","SP里的精确度"+accuracy);
    ...
    public static GeoLocationAPI getCellInfo(Context context){
        ...
        */
        ...
        Logger.I("AlxLocationManager","获取的基站信息是"+operator);
        if(operator==null || operator.length()<5){
            Logger.I("AlxLocationManager","获取基站信息有问题,可能是手机没插sim卡");
            return cellInfo;
        }
        ...
        if(cellLocation==null){
            Logger.I("AlxLocationManager","手机没插sim卡吧");
            return cellInfo;
        }
        if(mTelephonyManager.getPhoneType() == TelephonyManager.PHONE_TYPE_GSM) {
            Logger.I("AlxLocationManager","当前是gsm基站");
            GsmCellLocation location = (GsmCellLocation)cellLocation;
            lac = location.getLac();
            cellId = location.getCid();
            //这些东西非常重要，是根据基站获得定位的重要依据
            Logger.I("AlxLocationManager", " MCC移动国家代码 = " + mcc + "\t MNC移动网络号码 = " + mnc + "\t LAC位置区域码 = " + lac + "\t CID基站编号 = " + cellId);
        }else if(mTelephonyManager.getPhoneType() == TelephonyManager.PHONE_TYPE_CDMA) {
            // 中国电信获取LAC、CID的方式
            Logger.I("AlxLocationManager","现在是cdma基站");
            CdmaCellLocation location1 = (CdmaCellLocation) mTelephonyManager.getCellLocation();
            lac = location1.getNetworkId();
            cellId = location1.getBaseStationId();
            cellId /= 16;
        }else {
            Logger.I("AlxLocationManager","现在不知道是什么基站");
            return cellInfo;
        }
        ...
        if(Build.VERSION.SDK_INT<17) {//低版的android系统使用getNeighboringCellInfo方法
            ...
            if(infos==null){
                Logger.I("AlxLocationManager","手机型号不支持基站定位1");
                return cellInfo;
            }
            ...
            Logger.I("AlxLocationManager","基站信息是"+sb);
            ...
            if(infos!=null) {
                ...
                for (CellInfo i : infos) { // 根据邻区总数进行循环
                    Logger.I("AlxLocationManager", "附近基站信息是" + i.toString());//这里如果出现很多cid
                    GoogleCellTower tower = new GoogleCellTower();
                    if(i instanceof CellInfoGsm){//这里的塔分为好几种类型
                        Logger.I("AlxLocationManager","现在是gsm基站");
                        ...
                        Logger.I("AlxLocationManager", "现在是lte基站");
                        ...
                        Logger.I("AlxLocationManager","现在是wcdma基站");
    ...
    public static boolean isWifiEnvironment(Context context){
        ...
        if(networkInfo==null){
            Logger.I("AlxLocationManager","现在没有联网");
            return false;
        }
        int netType = networkInfo.getType();
        switch (netType){
            case ConnectivityManager.TYPE_WIFI:
                Logger.I("AlxLocationManager","现在是wifi网络,可以用ip定位");
                return true;
            case ConnectivityManager.TYPE_VPN://这个基本没用
                Logger.I("AlxLocationManager","现在是VPN网络");
                break;
            case ConnectivityManager.TYPE_MOBILE:
                Logger.I("AlxLocationManager","现在是移动网络,不能用ip定位");
                int subType = networkInfo.getSubtype();
                Logger.I("AlxLocationManager","移动网络子类是"+subType+"  "+networkInfo.getSubtypeName());//能判断是2g/3g/4g网络
                break;
            default:
                Logger.I("AlxLocationManager","不知道现在是什么网络");
                break;
        }
        return false;
    }
    ...
    public static boolean considerIP(Context context){
        ...
        if(Build.VERSION.SDK_INT<21) {//旧版本安卓获取网络状态
            ...
            for(NetworkInfo i:networkInfos){
                if(i==null)continue;
                Logger.I("AlxLocationManager","现在的网络是"+i.getTypeName()+i.getType()+"   "+i.getSubtypeName());//WIFI,VPN,MOBILE+LTE
                if(i.getType()== ConnectivityManager.TYPE_VPN){
                    Logger.I("AlxLocationManager","现在用的是VPN网络，不能用ip定位");
                    considerIP = false;
                    break;
                }
            }
            ...
            for(Network n:networks){
                ...
                Logger.I("AlxLocationManager","现在的网络是"+networkInfo.getTypeName()+networkInfo.getType()+"   "+networkInfo.getSubtypeName());//WIFI,VPN,MOBILE+LTE
    ...
    public static String determine2g3g4g(Context context){
        ...
        if(Build.VERSION.SDK_INT<21) {//旧版本安卓获取网络状态
            ...
            for(NetworkInfo i:networkInfos){
                if(i==null)continue;
                Logger.I("AlxLocationManager","正在查看当前网络的制式"+i.getTypeName()+i.getType()+"   "+i.getSubtypeName());//WIFI,VPN,MOBILE+LTE
                if(i.getType()!= ConnectivityManager.TYPE_MOBILE)continue;//只看流量
                else Logger.I("AlxLocationManager","现在是移动网络");
            ...
            for(Network n:networks){
                ...
                NetworkInfo networkInfo = connectivityManager.getNetworkInfo(n);
                if(networkInfo==null)continue;
                Logger.I("AlxLocationManager","正在查看当前网络的制式"+networkInfo.getTypeName()+networkInfo.getType()+"   "+networkInfo.getSubtypeName());//WIFI,VPN,MOBILE+LTE
    ...
    public static String determine2g3g4g(NetworkInfo info){
        ...
        switch (info.getSubtype()){
            case TelephonyManager.NETWORK_TYPE_LTE:
                Logger.I("AlxLocationManager","手机信号是lte");
                return "LTE";
            case TelephonyManager.NETWORK_TYPE_EDGE:
                Logger.I("AlxLocationManager","手机信号是edge");
                return "EDGE";
    ...
    public static GeoLocationAPI getWifiInfo(Context context, GeoLocationAPI geoLocationAPI){
        ...
        Logger.I("AlxLocationManager","准备开始扫描附近wifi");
        ...
        if(lsScanResult == null){
            Logger.I("AlxLocationManager","搜索附近wifi热点失败");
            return geoLocationAPI;
        }
        for (ScanResult result : lsScanResult) {
            Logger.I("AlxLocationManager","发现一个附近的wifi::"+result.SSID+"  mac地址是"+result.BSSID+"   信号强度是"+result.level);
    ...
    public static short getChannelByFrequency(int frequency) {
        ...
        Logger.I("AlxLocationManager","信道是"+channel);
        return channel;
    }
    ...
    public void sendJsonByPost(String json, String url){
        ...
        HttpRequest.post(url, params, new BaseHttpRequestCallback<String>(){
            ...
            protected void onSuccess(String s) {
                ...
                String result = s;
                Logger.I("AlxLocationManager","成功"+s);
                ...
                if(result==null || result.length()<10 || !result.startsWith("{")){
                    Logger.I("AlxLocationManager","返回格式不对"+result);
                }
                ...
                try {
                    ...
                    if(location==null){
                        Logger.I("AlxLocationManager","条件不足，无法确定位置");
                        return;
                    }
                    double latitude = location.getDouble("lat");
                    double longitute = location.getDouble("lng");
                    double google_accuracy = returnJson.getDouble("accuracy");
                    Logger.I("AlxLocationManager","谷歌返回的经纬度是"+latitude+"  :  "+longitute+"  精度是"+google_accuracy);
                    ...
                    Log.i("AlxLocationManager","条件不足，无法确定位置2",e);
                }
            ...
            public void onFailure(int errorCode, String msg) {
                ...
                Logger.I("AlxLocationManager","失败了"+msg+"   "+errorCode);
    ...
    public void checkNetCardState(WifiManager mWifiManager) {
        if (mWifiManager.getWifiState() == 0) {
            Logger.I("AlxLocationManager", "网卡正在关闭");
        } else if (mWifiManager.getWifiState() == 1) {
            Logger.I("AlxLocationManager", "网卡已经关闭");
        } else if (mWifiManager.getWifiState() == 2) {
            Logger.I("AlxLocationManager", "网卡正在打开");
        } else if (mWifiManager.getWifiState() == 3) {
            Logger.I("AlxLocationManager", "网卡已经打开");
        } else {
            Logger.I("AlxLocationManager", "没有获取到状态");
        }
    }
    ...
    public static void  getConnectedWifiInfo(WifiManager wifiManager){
        ...
        if (wifiConnection != null) {//获取当前链接的wifi信息，没什么用
            ...
            Logger.I("AlxLocationManager","手机的mac地址是"+mac);
        }
    }
    ...
    public static double getGPSDistance(double lat_a, double lng_a, double lat_b, double lng_b) {
        ...
        double distance = (2 * EARTH_RADIUS * Math.asin(Math.sqrt(2 - 2 * Math.cos(x1)
                ...
                * Math.sin(x2)) / 2));
        Logger.I("AlxLocationManager","位置发生了移动，距离是"+distance);
